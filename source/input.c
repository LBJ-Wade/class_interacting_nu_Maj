...
int input_read_parameters(
  ...
  if ((flag1 == _TRUE_) && (N_ncdm > 0)){
    ...
    /* VP: Read effective interaction rate with the majoron*/
    class_read_list_of_doubles_or_default("Gamma_eff_ncdmphi",pba->Gamma_eff_ncdmphi,0.0,N_ncdm);
    /* VP: Alternatively, read majoron decay width*/
    /* we will check later that both are not defined, otherwise we define one from the other */
    class_read_list_of_doubles_or_default("Gamma_phi",pba->Gamma_phi,0.0,N_ncdm);
    // /*VP: Read M_phi in units of eV */
    if(pba->Gamma_phi[N_ncdm-1]!=0 || pba->Gamma_eff_ncdmphi[N_ncdm-1]!=0){
    ...
    /* VP: read background ncdm distribution, 0 is fermi_dirac, 1 is fermi dirac v2 (ours), 2 is majoron. */
    class_read_list_of_integers_or_default("ncdm_background_distribution",pba->ncdm_background_distribution,0,N_ncdm);

    // class_read_double("M_phi_in_eV",pba->M_phi);
    // if(pba->M_phi == 0.0){
    //   //try to read in keV?
    ...
    /*VP: Check for duplicate entry of Gamma_eff_ncdmphi/Gamma_phi and define accordingly*/
    for(n=0; n<N_ncdm; n++){
      ...
      /*VP*/
      /*VP: Read entry corresponding to M_phi in the n_ncdm table and attribute it to Mphi*/
      // class_read_int("entry_is_M_phi",pba->entry_is_M_phi);
      // class_test(pba->entry_is_M_phi>=N_ncdm,errmsg,"the value of entry_is_M_phi is greater than N_ncdm. Please correct your ini file.")



      class_test(pba->Gamma_phi[n]!=0 && pba->Gamma_eff_ncdmphi[n]!=0,errmsg,
      ...
      if(pba->Gamma_phi[n]!=0.0){
        class_test(pba->M_phi==0.0,errmsg,"M_phi is zero but you have Gamma_phi nonzero. Please specify a value for M_phi.")
        pba->Gamma_eff_ncdmphi[n]= pba->Gamma_phi[n]/(3.18e-28*pba->M_phi*pba->M_phi*pow(1-4*pba->m_ncdm_in_eV[pba->entry_is_M_phi-1]*pba->m_ncdm_in_eV[pba->entry_is_M_phi-1]/pba->M_phi/pba->M_phi,0.5));
      }
